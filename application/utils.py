...
def base36encode(number, alphabet='0123456789abcdefghijklmnopqrstuvwxyz'):
    ...
    Reference: http://en.wikipedia.org/wiki/Base_36#Python%5FConversion%5FCode
    >>> base36encode(0)
    '0'
...
def random_hash_key():
    ...
    """
    # 60466175 is 'zzzzz'
    return ('0000' + base36encode(randint(0, 60466175)))[-5:]
    ...
    if isinstance(text, unicode):
        text = unicode(text.encode('utf-8')\
                .translate(string.maketrans("", ""),
                  string.punctuation).lower(), 'utf-8')
    else:
...
def scrubemail(data, rot13=False, css_junk=None):
    ...
    u'Send email to <a href="mailto:test@example.com">test@example.com</a>
    and you are all set.'
    >>> scrubemail(u"Send email to test@example.com and you are all set.",
                    rot13=True)
    u'Send email to <a class="rot13" data-href="znvygb:grfg@rknzcyr.pbz">
        test@example.com</a> and you are all set.'
    >>> scrubemail(u"Send email to test@example.com and you are all set.",
                    rot13=True, css_junk='z')
    u'Send email to <a class="rot13" data-href="znvygb:grfg@rknzcyr.pbz">
    test&#64;<span class="z">no</span>examp<span class="z">spam</span>le.com
    </a> and you are all set.'
    >>> scrubemail(u"Send email to test@example.com and you are all set.",
                    rot13=False, css_junk=('z', 'y'))
    u'Send email to <a href="mailto:test@example.com"><span class="y">
        test&#64;</span><span class="z">no</span><span class="y">examp
        </span><span class="z">spam</span><span class="y">le.com</span>
        </a> and you are all set.'
    """
    ...
    def convertemail(m):
        ...
        if css_junk and len(email) > 3:
            ...
            parts = (email[:third], email[third:third * 2],
                        email[third * 2:])
            if isinstance(css_junk, (tuple, list)):
                ...
                email = ('%s<span class="%s">no</span>%s<span class="%s">'
                            'spam</span>%s') % (
...
def get_word_bag(text):
    """
    Return a string containing all unique words in the given text, in
    alphabetical order.
