...
def has_post_stats(post):
    ...
    return is_siteadmin or post.admin_is(g.user) or (current_auth.is_authenticated and g.user.flags.get('is_employer_month'))
...
def load_user_data(user):
    ...
    if current_auth.is_authenticated or g.anon_user and not g.esession:
...
def record_views_and_events(response):
    ...
    if g.esession:  # Will be None for anon static requests
        if current_auth.is_authenticated or g.anon_user:
