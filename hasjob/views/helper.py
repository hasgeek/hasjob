...
ALLOWED_TAGS = ['strong', 'em', 'p', 'ol', 'ul', 'li', 'br', 'a']
...
def load_user_data(user):
    ...
    g.esession = None
    g.viewcounts = {}
    # Retrieve from cookie session if present there
    g.impressions = session.pop('impressions', {})
    g.campaign_views = []
    ...
    if request.endpoint not in ('static', 'baseframe.static'):
        ...
        if user:
            if (
                'au' in session
                and session['au'] is not None
                and not str(session['au']).startswith('test')
            ):
    ...
    if g.user or g.anon_user and not g.esession:
        g.esession = EventSession.get_session(
            uuid=session.get('es'), user=g.user, anon_user=g.anon_user
        )
    if g.esession:
    ...
    if g.anon_user:
        ...
        if 'impressions' in session:
            ...
            save_impressions(
                g.esession.id, list(session.pop('impressions').values()), now
            )
...
def session_jobpost_ab():
    ...
    result = {
        ji.jobpost_id: ji.bgroup
        for ji in JobImpression.query.filter_by(event_session=g.esession)
    }
    result.update(
        {
            jvs.jobpost_id: jvs.bgroup
            for jvs in JobViewSession.query.filter_by(event_session=g.esession)
        }
    )
    return result
...
def set_max_counts():
    ...
    if not postids:
        return {'max_impressions': 0, 'max_views': 0, 'max_opens': 0, 'max_applied': 0}
...
def make_pay_graph(currency, posts, rmin=None, rmax=None, minposts=5):
    ...
    pay_data = [
        (post.pay_cash_min, post.pay_cash_max)
        for post in posts
        if post.pay_type == PAY_TYPE.RECURRING
    ]
