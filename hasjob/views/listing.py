...
def jobdetail(domain, hashid):
    ...
    if post.state.UNPUBLISHED:
        if not (g.user and post.current_roles.admin):
            abort(403)
    if post.state.GONE:
        abort(410)
    if g.user:
    ...
    if is_siteadmin or post.current_roles.admin or (g.user and g.user.flags.get('is_employer_month')):
...
def withdraw(domain, hashid, key):
    ...
    if not ((key is None and g.user is not None and post.current_roles.admin) or (key == post.edit_key)):
        abort(403)
