...
def jobdetail(domain, hashid):
    ...
    if post.state.UNPUBLISHED:
        if not (current_auth.is_authenticated and post.admin_is(g.user)):
            abort(403)
    if post.state.GONE:
        abort(410)
    ...
    if current_auth.is_authenticated and not g.kiosk:
    ...
    if is_siteadmin or post.admin_is(g.user) or (current_auth.is_authenticated and g.user.flags.get('is_employer_month')):
...
def job_related_posts(domain, hashid):
    ...
    if is_siteadmin or (current_auth.is_authenticated and g.user.flags.get('is_employer_month')):
...
def applyjob(domain, hashid):
    ...
    if job_application:
        ...
        if applyform.validate_on_submit():
            if current_auth.is_authenticated and g.user.blocked:
...
def withdraw(domain, hashid, key):
    ...
    if not ((key is None and current_auth.is_authenticated and post.admin_is(g.user)) or (key == post.edit_key)):
        abort(403)
