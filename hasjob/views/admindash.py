...
from ..models import db, POSTSTATUS, EMPLOYER_RESPONSE, CreditTransaction
from ..forms import GrantCreditsForm
...
@app.route('/admin/dashboard/tools')
@lastuser.requires_permission('siteadmin')
def admin_tools():
    return render_template('admin_tools.html')


@app.route('/admin/dashboard/tools/credits', methods=['GET', 'POST'])
@lastuser.requires_permission('siteadmin')
def admin_tools_grant_credit():
    ...
    if form.validate_on_submit():
        for user in form.users.data:
            db.session.add(CreditTransaction(
                ...
                memo=form.memo.data))
        db.session.commit()
        return render_redirect(url_for('admin_tools'), code=303)
    return render_form(form=form, title=_(u"Grant credits…"), submit=_("Grant"),
        cancel_url=url_for('admin_tools'), ajax=False)
