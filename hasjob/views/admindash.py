...
def admin_dashboard_historical_userdays(q):
    if q == 'a':
        userdays = db.session.query('month', 'users', 'centile').from_statement(
            '''SELECT month, COUNT(user_id) AS users, PERCENTILE_CONT(ARRAY [0.5, 0.6, 0.7, 0.8, 0.9, 0.91, 0.92, 0.93, 0.94, 0.95, 0.96, 0.97, 0.98, 0.99, 1.0]) WITHIN GROUP (ORDER BY count) AS centile FROM (SELECT month, COUNT(day) AS count, user_id FROM (SELECT DATE_TRUNC('month', userjobview.created_at) AS month, DATE_TRUNC('day', userjobview.created_at) AS day, user_id FROM userjobview GROUP BY month, day, user_id) AS month_day_user GROUP BY month, user_id ORDER BY month, user_id) AS daycounts WHERE month > '2013-01-01' GROUP BY month ORDER BY month;''')
    elif q == 'c':
        userdays = db.session.query('month', 'users', 'centile').from_statement(
            '''SELECT month, COUNT(user_id) AS users, PERCENTILE_CONT(ARRAY [0.5, 0.6, 0.7, 0.8, 0.9, 0.91, 0.92, 0.93, 0.94, 0.95, 0.96, 0.97, 0.98, 0.99, 1.0]) WITHIN GROUP (ORDER BY count) AS centile FROM (SELECT month, COUNT(day) AS count, user_id FROM (SELECT DATE_TRUNC('month', userjobview.created_at) AS month, DATE_TRUNC('day', userjobview.created_at) AS day, userjobview.user_id FROM userjobview WHERE userjobview.user_id IN (SELECT DISTINCT(user_id) FROM job_application) GROUP BY month, day, user_id) AS month_day_user GROUP BY month, user_id ORDER BY month, user_id) AS daycounts WHERE month > '2013-01-01' GROUP BY month ORDER BY month;''')
    elif q == 'r':
        userdays = db.session.query('month', 'users', 'centile').from_statement(
            '''SELECT month, COUNT(user_id) AS users, PERCENTILE_CONT(ARRAY [0.5, 0.6, 0.7, 0.8, 0.9, 0.91, 0.92, 0.93, 0.94, 0.95, 0.96, 0.97, 0.98, 0.99, 1.0]) WITHIN GROUP (ORDER BY count) AS centile FROM (SELECT month, COUNT(day) AS count, user_id FROM (SELECT DATE_TRUNC('month', userjobview.created_at) AS month, DATE_TRUNC('day', userjobview.created_at) AS day, userjobview.user_id FROM userjobview WHERE userjobview.user_id IN (SELECT DISTINCT(user_id) FROM job_application WHERE response=:response) GROUP BY month, day, user_id) AS month_day_user GROUP BY month, user_id ORDER BY month, user_id) AS daycounts WHERE month > '2013-01-01' GROUP BY month ORDER BY month;''').params(
