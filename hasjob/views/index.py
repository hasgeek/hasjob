...
def index(basequery=None, filters={}, md5sum=None, tag=None, domain=None, location=None, title=None, showall=True, statusfilter=None, batched=True, ageless=False, cached=False, query_string=None, filterset=None, template_vars={}):
    ...
    if basequery is None:
        ...
        is_index = False
    if basequery is None and not (current_auth.is_authenticated or g.kiosk or (board and not board.require_login)):
        showall = False
        batched = False
    ...
    if not location and 'l' not in request.args and g.user_geonameids and (current_auth.is_authenticated or g.anon_user) and (
